...
export function InvoiceHistory({
  ...
  return (
    <>
      <Dialog open={isOpen} onOpenChange={setIsOpen}>
        <DialogContent className="sm:max-w-[900px] w-[95vw] h-[90vh] overflow-hidden bg-white flex flex-col">
          <DialogHeader>
            <DialogTitle className="text-2xl font-bold text-gray-800">Invoice History</DialogTitle>
            <DialogDescription className="text-gray-600">
              All invoices generated by {businessDetails.name}
                ...
                <AnimatePresence>
                  {filteredInvoices.map((invoice, index) => (
                    invoice && invoice.invoice_number ? (
                      ...
                      >
                        <Card className="h-full border-b border-gray-200 last:border-b-0">
                          <CardContent className="p-4">
                            <h3 className="text-lg font-semibold mb-2">{invoice.invoice_number}</h3>
                            <p className="text-sm text-gray-600 mb-1">Date: {formatDate(invoice.billDate)}</p>
                            <p className="text-sm font-medium mb-1">Amount: ₹{invoice.total_amount.toFixed(2)}</p>
                            <span className={`px-2 py-1 rounded-full text-xs font-semibold ${
                              invoice.paymentStatus === 'paid' ? 'bg-green-100 text-green-800' :
                              invoice.paymentStatus === 'due' ? 'bg-red-100 text-red-800' :
